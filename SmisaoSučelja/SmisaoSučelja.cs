...
namespace Vsite.CSharp.Sučelja
{
    class SmisaoSučelja
    {
        ...
        private class SortByName : IComparer<Osoba> //implementira klasu-sučelje
        {
        ...
        private static int SortCompareByDate(Osoba x, Osoba y)
        {
        ...
        public static void SortiranoPoDatumuRođenja(List<Osoba> osobe)
        {
            ...
            osobe.Sort(SortCompareByDate);
        }

        public static void SortiranoPoMjestuRođenja(List<Osoba> osobe)
        {
            // lambda izraz:012 Koristeći preopterećenu inačicu metode List<T>.Sort(Comparison<T>) osobe sortirati prema njihovim mjestima rođenja.
